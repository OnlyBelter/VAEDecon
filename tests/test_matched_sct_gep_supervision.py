...
def test_write_true_sct_gep_targets_into_uses_preindexed_rows_and_reports_count():
    ...
    true_sct_gep_dest = np.zeros((3, 2, 2), dtype=np.float32)
    true_sct_gep_present_mask_dest = np.zeros((3, 2), dtype=bool)

    written = _write_true_sct_gep_targets_into(
        raw_bulk_sample_ids=["sample_1", "sample_2", "sample_3"],
        row_positions=np.asarray([0, 1, 2], dtype=np.int64),
        cell_types=["CT1", "CT2"],
        filtered_mapping_df=filtered_mapping_df,
        aligned_sct_geps_df=aligned_sct_geps_df,
        true_sct_gep_dest=true_sct_gep_dest,
        true_sct_gep_present_mask_dest=true_sct_gep_present_mask_dest,
    )

    assert written == 3
    assert true_sct_gep_present_mask_dest.tolist() == [
        [True, True],
        [True, False],
        [False, False],
    ]
