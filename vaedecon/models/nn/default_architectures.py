...
from vaedecon.models.nn import BaseDecoder, BaseDiscriminator, BaseEncoder, BaseMetric
...
class Encoder_VAE_MLP(BaseEncoder):
    ...
    def forward(self, x, output_layer_levels: List[int] = None) -> ModelOutput:
        output = ModelOutput()
        ...
        out = x.reshape(-1, np.prod(self.input_dim))
        embedding = torch.ones(self.latent_dim)
        log_var = torch.ones(self.latent_dim)
        ...
        for i in range(max_depth):
            ...
            if i + 1 == self.depth:
                output["embedding"] = self.embedding(out)
                output["log_var"] = self.log_var(out)
        # output = ModelOutput(embedding=embedding, log_var=log_var, **output)
