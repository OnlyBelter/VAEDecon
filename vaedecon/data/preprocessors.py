...
class DataProcessor:
    ...
    **minimal shape**.
    ...
    def process_data(
        self, data: Union[np.ndarray, torch.Tensor, anndata.AnnData], batch_size: int = 128
    ) -> torch.Tensor:
        ...
        if isinstance(data, np.ndarray) or torch.is_tensor(data):
            ...
            data = self._process_data_anndata(data, batch_size=batch_size)
    ...
    def _process_data_array(self, data: np.ndarray, batch_size: int = 100) -> torch.Tensor:
        ...
        for i in range(num_complete_batch):
            ...
            if self.has_nan(data[i * batch_size : (i + 1) * batch_size]):
                raise ValueError("Nan detected in input data!")

            processed_data = self.to_tensor(
        ...
        if num_in_last > 0:
            # Detect potential nan
            if self.has_nan(data[-num_in_last:]):
                raise ValueError("Nan detected in input data!")

            processed_data = self.to_tensor(data[-num_in_last:])
            full_data.append(processed_data)
    ...
    def _process_data_anndata(self, data: anndata.AnnData, batch_size: int = 100) -> torch.Tensor:
        x = data.X
